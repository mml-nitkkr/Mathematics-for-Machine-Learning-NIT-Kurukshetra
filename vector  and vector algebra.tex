...
\end{itemize}
Here note that magnitude of  any vector is " non- negative" value. Hopefully you know what does non negative means. If $a\in\mathbb{R}$ is non negative , it means that either $a>0$ or $a=0$. On the other hand if $a$ is positive means $a>0$ and cannot  be zero. 
\item Dot  Product: Dot product of two  vectors $ \overrightarrow{v}, \overrightarrow{u}$is denoted by $\overrightarrow{v}.\overrightarrow{u}$ and defined as  
